...
from models.losses import cross_entropy, focal_loss, cross_entropy_with_auxilary
...
class CDTrainer():
    ...
    def __init__(self, args, dataloaders):
        ...
        if args.loss == 'ce':
            ...
            self._pxl_loss = cross_entropy
            l1 = (int(args.loss[-2]))/10
            l2 = (int(args.loss[-1]))/10
            self.weight = torch.tensor([l1, l2]).to(self.device)
        elif args.loss == 'focal':
            self._pxl_loss = focal_loss
        elif args.loss == 'ce_aux':
            self._pxl_loss = cross_entropy_with_auxilary
        else:
    ...
    def _forward_pass(self, batch):
        self.batch = batch
        if self.pre_cat:
            img = np.concatenate((batch['A'], batch['B']), axis=1)
            self.G_pred = self.net_G(torch.tensor(img).to(self.device))
        else:
